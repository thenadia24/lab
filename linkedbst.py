...
class LinkedBST(AbstractCollection):
    ...
    def __str__(self):
        ...
        def recurse(node, level):
            result_string = ""
            if node is None:
                result_string += recurse(node.right, level + 1)
                result_string += "| " * level
                result_string += str(node.data) + "\n"
                result_string += recurse(node.left, level + 1)
    ...
    def height(self):
        ...
        def height1(root):
            ...
            left_ans = height1(root.left)
            right_ans = height1(root.right)
    ...
    def is_balanced(self):
        ...
        :return:
        '''
        result = (2 * log(self._size + 1, 2)) - 1 > self.height()
        return result
    ...
    def range_find(self, low, high):
        ...
        def sub_range_function(node):
            ...
            if node.data < high:
                result.append(sub_range_function(node.right))
            if node.data > low:
                result.append(sub_range_function(node.left))

            return result

        return sub_range_function(self._root)
    ...
    def demo_bst(path):
        ...
        words_to_find = random.sample(words, max_count)
